	...
	ESX.TriggerServerCallback('esx_boat:getGarage', function (ownedBoats)
		...
		if #ownedBoats == 0 then
			...
			for i=1, #ownedBoats, 1 do
				local boatVehicle = json.decode(ownedBoats[i].vehicle)
				local boatLabel = getVehicleLabelFromHash(boatVehicle.model)  .. " (" .. boatVehicle.plate .. ")"
				if(ownedBoats[i].state) then
					boatLabel = boatLabel .. " En garage"
				else
					boatLabel = boatLabel .. " En el mar"
				end
				table.insert(elements, {
					label = boatLabel,
					vehicleProps = boatVehicle,
					stored = ownedBoats[i].state
				})
			end
			...
			function (data, menu)
				...
				if ESX.Game.IsSpawnPointClear(garage.SpawnPoint, 4.0) then
					...
					local spawn = false
					local found = false
					if data.current.stored  == false then
						print (vehicleProps.plate)
						local vehicles = ESX.Game.GetVehicles()
						if #vehicles > 0 then
							for k,v in ipairs(vehicles) do
								local ingameplate = ESX.Math.Trim(GetVehicleNumberPlateText(v))
								if ingameplate == ESX.Math.Trim(vehicleProps.plate) then
									found = true
									-- Make sure the vehicle we're deleting is empty, or else it wont be deleted
									if GetVehicleNumberOfPassengers(v) == 0 and IsVehicleSeatFree(v, -1) then
										ESX.Game.DeleteVehicle(v)
										spawn = true
										ESX.ShowNotification(_U('lucky_recover'))
									else
										TriggerEvent('ESX:showNotification', 'Tu vehículo no puede ser recuperado por la guardia costera ahora mismo')
									end
								end
							end
						end
...
function getVehicleLabelFromHash(hash)
	...
	return 'Modelo desconocido [' .. model .. ']'
end
