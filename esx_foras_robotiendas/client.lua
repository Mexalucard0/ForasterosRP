...
RegisterNetEvent('esx_foras_robotiendas:onPedDeath')
AddEventHandler('esx_foras_robotiendas:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('esx_foras_robotiendas:msgPolice')
AddEventHandler('esx_foras_robotiendas:msgPolice', function(store, robber)
    ESX.ShowAdvancedNotification(Config.Shops[store].name, Translation[Config.Locale]['robbery'], Translation[Config.Locale]['cop_msg'], "Dia_goon")
    while true do
...
RegisterNetEvent('esx_foras_robotiendas:removePickup')
AddEventHandler('esx_foras_robotiendas:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('esx_foras_robotiendas:robberyOver')
AddEventHandler('esx_foras_robotiendas:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('esx_foras_robotiendas:talk')
AddEventHandler('esx_foras_robotiendas:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('esx_foras_robotiendas:rob')
AddEventHandler('esx_foras_robotiendas:rob', function(i)
                ...
                Citizen.CreateThread(function()
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 12.0 then
                                if robbingPed == i then
                                    PlaySoundFrontend(-1, 'ROBBERY_MONEY_TOTAL', 'HUD_FRONTEND_CUSTOM_SOUNDSET', true)
                                    TriggerServerEvent('esx_foras_robotiendas:pickUp', i)
                                    robbingPed = nil
                                end
                                break
                            end
...
RegisterNetEvent('esx_foras_robotiendas:resetStore')
AddEventHandler('esx_foras_robotiendas:resetStore', function(i)
...
function _CreatePed(hash, coords, heading)
    ...
    SetPedFleeAttributes(ped, 0, 0)
    DecorSetBool(ped, "MRFIANOANIM", true)
    return ped
end
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('esx_foras_robotiendas:pedDead', i)
                end
            end
            Wait(5000)
        end
    ...
    while true do
        ...
        if IsPedArmed(me, 7) and ESX.PlayerData ~= nil and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name ~= 'police' then
            if IsPlayerFreeAiming(PlayerId()) or IsPedInMeleeCombat(me) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 12.0 then
                        if not robbing then
                            local canRob = nil
                            ESX.TriggerServerCallback('esx_foras_robotiendas:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(1)
                            ...
                            if canRob == true then
                                ...
                                TriggerServerEvent('esx_foras_robotiendas:alarm', i)
                                
                                local scared = 0
                                while scared < 200 and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 12.0 do
                                    local sleep = 1200 --300 = 1 minuto
                                    ...
                                    while sleep >= GetGameTimer() and not IsPedDeadOrDying(peds[i]) do
                                        Wait(0)
                                        DrawRect(0.5, 0.9, 0.2, 0.03, 198, 196, 197, 200)
                                        local draw = scared/1000
                                        DrawRect(0.5, 0.9, draw, 0.03, 255, 0, 44, 200)
                                    end
                                    scared = scared + 1
                                end
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 12.0 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        robbingPed = i
                                        TriggerServerEvent('esx_foras_robotiendas:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('esx_foras_robotiendas:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end
...
RegisterNetEvent("esx_foras_robotiendas:setblip")
AddEventHandler(
	"esx_foras_robotiendas:setblip",
    function(store)
        Citizen.CreateThread(function()
            PlaySoundFrontend(-1, "CHALLENGE_UNLOCKED", "HUD_AWARDS", 1)
            --RemoveBlip(blipRobbery)
            local blipRobbery = AddBlipForCoord(Config.Shops[store].coords)
