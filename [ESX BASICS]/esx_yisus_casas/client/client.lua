...
RegisterCommand('darcasa', function(source, args)
    if args[1] == nil or args[2] == nil then
        ESX.ShowNotification("Uso incorrecto, usa /darcasa (PID) (CID)")
    elseif ESX.GetPlayerData().job.name ~= "inmobiliario" then
        ESX.ShowNotification("No eres inmobiliario")
    elseif ESX.GetPlayerData().job.name == "inmobiliario" then
        TriggerServerEvent("esx_yisus_casas:darcasa", args[2], args[1])
        ESX.ShowNotification("Accion correcta")
    end
end)
                            ...
                            if not AvailableHouses[k] and OwnedHouse.houseId == 0 then
                                ...
                                function(data, menu)
                                    if data.current.value == 'yes' then
                                    	ESX.ShowNotification("Para comprar una casa ve a la inmobiliaria")
