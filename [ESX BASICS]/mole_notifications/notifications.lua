...
function MoleShowAdvancedNotification(title, subject, msg, icon, iconType, job)
    ...
    if(icon ~= nil)then
        advanceMsg = "<div ><table style='color:white;font-size:14px''><tr><td><img height=50 src='https://images.forasterosrp.com/notifications/" .. icon .. ".jpg'></td><td style='vertical-align:top'><div style='color:lime;font-size:16px'>" .. title .. "</div>" .. subject .. "</td></tr><tr><td colspan=2>" .. msg .. "</td></tr></table></div>"
    end
...
function MoleSendPrompt(options, jobs, cb) 
    ...
    print (options.text)
    local notificationId = MoleSendNotification(options, jobs)
...
RegisterNetEvent("mole_notifications:SendPrompt")
AddEventHandler("mole_notifications:SendPrompt", function(options, job, cb)
    MoleSendPrompt(options, job, cb)
end)
