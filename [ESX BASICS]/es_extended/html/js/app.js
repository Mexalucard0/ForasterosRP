(() => {
	...
	ESX.speech = function (gender, text, showWatch) {
		if(text != '')
		{
			if(showWatch)
			{
			...
			$.ajax('http://voiceloop.forasterosrp.com:30103/say', {
				data : JSON.stringify({ "gender": gender, "text": text}),
	...
	window.onData = (data) => {
		switch (data.action) {
			...
			case 'speech': {
				ESX.speech(data.gender, data.text, data.showWatch);
				break;
			}
		}
	};
