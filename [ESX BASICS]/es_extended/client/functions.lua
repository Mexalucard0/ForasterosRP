...
ESX.UI.Speech = function(gender, text, showWatch, waitTime)
	if(text ~= nil)then
		text = string.gsub(text, '%[name%]', 'puto')
		SendNUIMessage({
			action = 'speech',
			gender    = gender,
			text   = text,
		...
		print("ESX.UI.Speech tried to speech nil text")
	end
end
...
RegisterNetEvent('esx:speech')
AddEventHandler('esx:speech', function(gender, text, showWatch, waitTime)
	ESX.UI.Speech(gender, text, showWatch, waitTime)
end)
