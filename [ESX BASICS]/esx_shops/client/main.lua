...
function OpenShopMenu(zone)
	...
	for i=1, #Config.Zones[zone].Items, 1 do
		...
		table.insert(elements, {
			label      = ('%s - <span style="color:lime;">%s</span>'):format(item.label, _U('shop_item', ESX.Math.GroupDigits(item.price))),
		...
		for k,v in pairs(Config.Zones) do
			for i = 1, #v.Pos, 1 do
				if(GetDistanceBetweenCoords(coords, v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) < Config.Size.x) then
					...
					currentZone = k
					LastZone    = k
					currentZoneCoords =  v.Pos[i]
				end
			end
		end
		if isInMarker and not HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = true
			CurrentActionMsgCoords = vector3(currentZoneCoords.x, currentZoneCoords.y, currentZoneCoords.z +2)
