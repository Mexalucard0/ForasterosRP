...
Citizen.CreateThread(function()
	...
	local openMenu = function(namespace, name, data)
		TriggerEvent('esx_mole_status:rightbottomopacity', 0.2)
		SendNUIMessage({
			action    = 'openMenu',
			namespace = namespace,
			name      = name,
	...
	local closeMenu = function(namespace, name)
		TriggerEvent('esx_mole_status:rightbottomopacity', 1)
		SendNUIMessage({
			action    = 'closeMenu',
			namespace = namespace,
			name      = name,
	...
	RegisterNUICallback('menu_submit', function(data, cb)
		...
		if menu ~= nil and menu.submit ~= nil then
	...
	RegisterNUICallback('menu_cancel', function(data, cb)
		...
		if menu ~= nil and menu.cancel ~= nil then
	...
	Citizen.CreateThread(function()
		while true do
			...
			if IsControlPressed(0, 202) and IsInputDisabled(0) and (GetGameTimer() - GUI.Time) > 150 then --backaspace no, que incluye el mouse right
				SendNUIMessage({
					action  = 'controlPressed',
					control = 'BACKSPACE'
				})
