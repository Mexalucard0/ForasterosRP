...
RegisterServerEvent('esx_foras_farmacia:toofar')
AddEventHandler('esx_foras_farmacia:toofar', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_foras_farmacia:killblip', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('esx_foras_farmacia:toofarlocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Banks[robb].nameofbank)
	end
end)

RegisterServerEvent('esx_foras_farmacia:rob')
AddEventHandler('esx_foras_farmacia:rob', function(robb)
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('esx_foras_farmacia:setblip', xPlayers[i], Banks[robb].position)
					end
				end
				...
				TriggerClientEvent('esx_foras_farmacia:currentlyrobbing', source, robb)
				...
				SetTimeout(600000, function()
					...
					if(robbers[savedSource])then
						...
						rob = false
						TriggerClientEvent('esx_foras_farmacia:robberycomplete', savedSource, job)
						if(xPlayer)then
                            local randomitems = math.random(0,20)
							xPlayer.addInventoryItem('chillpill', randomitems)
                            xPlayer.addInventoryItem('libro_recetas', randomitems)
