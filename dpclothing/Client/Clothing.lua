...
local Drawables = {
	...
	["Shoes"] = {
		Drawable = 6,
		Table = { Standalone = true, Male = 34+84, Female = 35+84, Player_Zero = 1, Player_One = 15, Player_Two = 17 },
		Emote = { Dict = "random@domestic", Anim = "pickup_low", Move = 0, Dur = 1200 }
	},
	["Neck"] = {
		Drawable = 7,
		Table = { Standalone = true, Male = 0, Female = 0, Player_Zero = 0, Player_One = 0, Player_Two = 0 },
...
local Extras = {
	["Shirt"] = {
		Drawable = 11,
		Table = {
			Standalone = true, Male = 252+84, Female = 15,
			Extra = { 
	...
	["Pants"] = {
		Drawable = 4,
		Table = { Standalone = true, Male = 61 + 84, Female = 62 + 84, Player_Zero = 18, Player_One = 18, Player_Two = 26 },
...
local function PlayToggleEmote(e, cb, noAnim)
	if(noAnim)then
		cb()
	else
...
function ApplyClothing()
	local Ped = PlayerPedId()
	local tempEquiped = LastEquipped
	LastEquipped = {}
	for k,v in pairs(tempEquiped) do
		if(Drawables[k])then
			ToggleClothing(k, nil, true)
		end
		if(Props[k])then
			ToggleProps(k, true)
		end
...
function ToggleClothing(which, extra, noAnim)
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		for k,v in pairs(Table) do
			if not Toggle.Remember then
				if k == Cur.Drawable then
					PlayToggleEmote(Toggle.Emote, function() SetPedComponentVariation(Ped, Toggle.Drawable, v, Cur.Texture, 0) end, noAnim) return true
				end
			else
				if not LastEquipped[which] then
					if k == Cur.Drawable then
						PlayToggleEmote(Toggle.Emote, function() LastEquipped[which] = Cur SetPedComponentVariation(Ped, Toggle.Drawable, v, Cur.Texture, 0) end, noAnim) return true
					end
				else
					local Last = LastEquipped[which]
					PlayToggleEmote(Toggle.Emote, function() SetPedComponentVariation(Ped, Toggle.Drawable, Last.Drawable, Last.Texture, 0) LastEquipped[which] = false end, noAnim) return true
				end
			end
		end
		...
		if not LastEquipped[which] then
			if Cur.Drawable ~= Table then 
				if not Gender and  DoesEntityExist(customPedMask) then
					DetachEntity(customPedMask)
					DeleteEntity(customPedMask)
				end
			...
			if not Gender then
				customPedMask = CreateObject(GetHashKey("p_d_scuba_mask_s"),coords.x,coords.y,coords.z,true,true,true)
				AttachEntityToEntity(customPedMask, GetPlayerPed(-1), GetPedBoneIndex(GetPlayerPed(-1), 12844), 0, 0, 0, 0, 90.0, 180.0, true, true, false, true, 1, true) -- Attach object to head
			end
			local Last = LastEquipped[which]
			PlayToggleEmote(Toggle.Emote, function()
...
function ToggleProps(which, noAnim)
	if Cooldown then return end
	...
	if not Prop.Variants then
		if Cur.Prop ~= -1 then -- If we currently are wearing this prop, remove it and save the one we were wearing into the LastEquipped table.
			PlayToggleEmote(Prop.Emote.Off, function() LastEquipped[which] = Cur ClearPedProp(Ped, Prop.Prop) end, noAnim) return true
		else
			local Last = LastEquipped[which] -- Detect that we have already taken our prop off, lets put it back on.
			if Last then
				PlayToggleEmote(Prop.Emote.On, function() SetPedPropIndex(Ped, Prop.Prop, Last.Prop, Last.Texture, true) end, noAnim) LastEquipped[which] = false return true
			end
		end
		...
		for k,v in pairs(Variations) do
			if Cur.Prop == k then
				PlayToggleEmote(Prop.Emote.On, function() SetPedPropIndex(Ped, Prop.Prop, v, Cur.Texture, true) end, noAnim) return true
			end
		end
...
local isFirstModelLoad = true
RegisterNetEvent('skinchanger:modelLoaded')
AddEventHandler('skinchanger:modelLoaded', function(xPlayer)
	if(isFirstModelLoad)then
		isFirstModelLoad = false
		ToggleClothing("Mask", nil, true)
		ToggleClothing("Gloves", nil, true)
		ToggleProps("Hat", true)
	end
end)

AddEventHandler('onResourceStart', function(resource) -- Mostly for development, restart the resource and it will put all the clothes back on.
	if resource == GetCurrentResourceName() then
		while ESX == nil do
			Citizen.Wait(10)
		end
