...
AddEventHandler('esx_mole_fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if currentFuel > 100.0 then
			currentFuel = 100.0
			isFueling = false
			TriggerEvent('esx_mole_fuel:onFuelingChanged', isFueling)
		end
	...
	if pumpObject then
		TriggerServerEvent('esx_mole_fuel:pay', currentCost)
	end
...
AddEventHandler('esx_mole_fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	TriggerEvent('esx_mole_fuel:startFuelUpTick', pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			if Config.UseESX then
				extraString = "<br>" .. Config.Strings.TotalCost .. ": ~g~$" .. Round(currentCost, 1)
			end
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								...
								if IsControlJustReleased(0, 38) then
									isFueling = true
									TriggerEvent('esx_mole_fuel:onFuelingChanged', isFueling)
									TriggerEvent('esx_mole_fuel:refuelFromPump', isNearPump, ped, vehicle)
									LoadAnimDict("timetable@gardener@filling_can")
								end
							else
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								TriggerServerEvent('esx_mole_fuel:pay', Config.JerryCanCost)
							...
							if Config.UseESX then
								...
								if refillCost > 0 then
									if currentCash >= refillCost then
										...
										if IsControlJustReleased(0, 38) then
											TriggerServerEvent('esx_mole_fuel:pay', refillCost)
...
RegisterNetEvent('esx_mole_fuel:SetFuel')
AddEventHandler('esx_mole_fuel:SetFuel', function(vehicle, fuel)
