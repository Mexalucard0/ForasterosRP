...
function IsMpPed(ped)
	...
	local Player_One = GetHashKey("player_one")
	local Player_Two =  GetHashKey("player_two")
	
	local CurrentModel = GetEntityModel(ped)

	for k, v in pairs(Config.AddonAllowedPeds) do
		local AddonModel = GetHashKey(v)

		if CurrentModel == Male then 
			return "Male" 
		elseif CurrentModel == Female then 
			return "Female" 
		elseif CurrentModel == Player_Zero then 
			return "Player_Zero" 
		elseif CurrentModel == Player_One then 
			return "Player_One" 
		elseif CurrentModel == Player_Two then 
			return "Player_Two" 
		elseif CurrentModel == AddonModel then
			return v
		end
	end
...
RegisterNetEvent('dpc:ApplyClothing')
AddEventHandler('dpc:ApplyClothing', function()
	ApplyClothing()
end)
